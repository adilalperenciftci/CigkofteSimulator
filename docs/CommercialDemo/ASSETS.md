...
## Where art may come from

Three sources, in this order, set by the project owner on 2026-08-07. Nothing
else is acquired, and nothing is bought.

1. **The owner's own Fab library.** Things already owned. The audio side of that
   library is inventoried below; the art side is checked per need rather than
   listed wholesale, because it changes.
2. **Free assets, researched and licence-checked.** CC0 and equivalent, with the
   licence recorded in this file before the file enters the repository. A
   permissive-looking source page is not a licence; the terms get read.
3. **Modelled here, in Blender.** When neither of the above has it. This is the
   fallback rather than the first move, because a made asset costs time the
   systems work also wants — but it is always available, so no missing asset
   blocks a stage.

What this rules out: paid marketplace purchases, anything whose licence forbids
commercial use or redistribution, and anything generated by a service whose
output terms have not been read. `docs/CreativePipelines` and the
`game-*-pipeline` skills carry the intake and hashing procedure.
